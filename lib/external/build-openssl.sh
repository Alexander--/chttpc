...
export CFLAGS="-D__ANDROID_API__=15 -pipe -fPIC -Os -g0"
...
if [ "$_ANDROID_ARCH" == "arch-x86" ]; then
	...
	./Configure android-x86 -no-cast no-comp no-shared no-hw no-engine --cross-compile-prefix="$CROSS_COMPILE" --openssldir="$A2_ROOT" $CFLAGS
